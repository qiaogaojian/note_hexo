...
- Android 优化：网络优化、卡顿优化、内存优化、瘦包、内存泄漏、ANR、Native Crash其他：解析 XML、进程保活、播放器、Lint、CI、CD、AOP、JetPack
...
- SurfaceTexture：将图像流转为 OpenGL 外部纹理，不直接显示GLSurfaceView：加入 EGL 管理，自带 GL 上下文和 GL 渲染线程
#### 事件分发机制
